...
const ingredientesN1_impossiveis = [
    ...
    "alface", "sushi", "esparguete", "melancia",
    "Molho", "Queijo", "Massa" // capitalized to test duplicate rule
];
...
function validarFSM_N1(tok, pos) {
    ...
    if (ingredientesN1_impossiveis_norm.includes(tok)) {
        // desenha o ícone normalizado para feedback visual
    ...
    function redrawTransitions() {
        ...
        for (const e1 in fsm.trans) {
            for (const tok in fsm.trans[e1]) {
                ...
                // Place label on the opposite side for better readability
                const dx = x2 - x1;
                const dy = y2 - y1;
                ...
                if (length > 0) {
                    // Position label perpendicular to the line, on the right side
                    const offset = 60;
...
function adicionarTransicao() {
    ...
    inpTok.type = "text";
    inpTok.placeholder = "ingrediente";
    ...
    inpTok.addEventListener("input", desenharAutomatoN3);
