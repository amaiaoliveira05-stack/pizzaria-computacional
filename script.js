...
function iniciarNivel(n) {
    ...
    if (n === 1) {
        document.getElementById("titulo-nivel").innerText = "Desafio 1 — Criar Pizza";
    ...
    if (n === 2) {
        document.getElementById("titulo-nivel").innerText = "Desafio 2 — Autómato Aleatório";
        iniciarNivel2();
        document.getElementById("nivel2").classList.remove("hidden");
    }

    if (n === 3) {
        document.getElementById("titulo-nivel").innerText = "Desafio 3 — Criar Autómato";
...
function getAjudaForNivel(n) {
    if (n === 1) {
        return `
            <p><strong>Desafio 1: Criar Pizza</strong></p>
            <ul>
        ...
        return `
            <p><strong>Desafio 2: Autómato Aleatório</strong></p>
            <ul>
        ...
        return `
            <p><strong>Desafio 3: Criar Autómato</strong></p>
            <ul>
...
function mostrarSaberMais() {
    const modal = document.getElementById("modal-saber-mais");
    const content = document.getElementById("conteudo-saber-mais");
    content.innerHTML = getSaberMaisForNivel(nivelAtual);
    modal.classList.remove("hidden");
}

function fecharSaberMais() {
    document.getElementById("modal-saber-mais").classList.add("hidden");
}

function getSaberMaisForNivel(n) {
    if (n === 1) {
        return `
            <p><strong>Desafio 1: Criar Pizza</strong></p>
            <p>Este desafio simula a criação de uma pizza seguindo regras de um autómato finito determinístico (AFD). O objetivo é construir uma pizza válida adicionando ingredientes na ordem correta.</p>
            <p><strong>Conceitos Computacionais:</strong></p>
            <ul>
                <li><strong>Autómato Finito Determinístico (AFD):</strong> Um modelo matemático que processa sequências de símbolos de acordo com regras de transição entre estados.</li>
                <li><strong>Estados:</strong> Representam as fases do processo de criação da pizza (ex: S0 - início, S1 - massa adicionada, etc.).</li>
                <li><strong>Transições:</strong> Mudanças de estado provocadas por ações (adicionar ingredientes).</li>
                <li><strong>Estado Inicial e Final:</strong> O processo começa no estado inicial e deve terminar num estado final para ser válido.</li>
                <li><strong>Expressão Regular:</strong> Define a sequência válida de ingredientes usando notação formal.</li>
            </ul>
        `;
    } else if (n === 2) {
        return `
            <p><strong>Desafio 2: Autómato Aleatório</strong></p>
            <p>Neste desafio, você interage com um autómato gerado aleatoriamente. O objetivo é alcançar o estado final seguindo as transições permitidas.</p>
            <p><strong>Conceitos Computacionais:</strong></p>
            <ul>
                <li><strong>Autómato Aleatório:</strong> Um AFD gerado proceduralmente com estados e transições aleatórias.</li>
                <li><strong>Transições Não-Determinísticas:</strong> Em dificuldades médias e difíceis, podem existir múltiplas transições para o mesmo símbolo.</li>
                <li><strong>Estado Atual:</strong> O estado em que o autómato se encontra, destacado visualmente.</li>
                <li><strong>Caminho Válido:</strong> Sequência de transições que leva do estado inicial ao final.</li>
                <li><strong>Piscar de Ajudas:</strong> Indica ingredientes válidos no estado atual (se ativado).</li>
            </ul>
        `;
    } else if (n === 3) {
        return `
            <p><strong>Desafio 3: Criar Autómato</strong></p>
            <p>Aqui você deve criar seu próprio autómato finito determinístico que aceite uma sequência específica de ingredientes.</p>
            <p><strong>Conceitos Computacionais:</strong></p>
            <ul>
                <li><strong>Construção de AFD:</strong> Definir estados, estado inicial, estados finais e transições.</li>
                <li><strong>Sequência de Entrada:</strong> Cadeia de símbolos que o autómato deve aceitar.</li>
                <li><strong>Tabela de Transições (δ):</strong> Matriz que define como o autómato muda de estado.</li>
                <li><strong>Diagrama Visual:</strong> Representação gráfica do autómato com estados e transições.</li>
                <li><strong>Validação:</strong> Verificar se o autómato aceita exatamente a sequência fornecida.</li>
            </ul>
        `;
    }
    return "Informação adicional não disponível.";
}
...
function criarIngredientesN1() {
    ...
    filtered.forEach(ing => {
        ...
        iconDiv.style.margin = "0 auto 6px auto";
...
function getNextAvailableIngredients() {
    ...
    switch (estadoN1) {
        ...
        case "S1":
            // After massa: molho or queijo, but for hard, only molho
            if (dificuldadeN1 === "dificil") {
                available.push("molho");
            } else {
        ...
        case "S1":
            if (dificuldadeN1 === "dificil") {
                if (tok === "molho") {
                    ...
                    return logN1("⛔ Para dificuldade difícil, tens de adicionar molho primeiro.");
                }
            } else {
                if (tok === "molho") {
                    estadoN1 = "S1.5";
...
function mostrarFormalizacao_N1() {
    ...
    // Definir o autómato do nível 1 baseado na dificuldade
    let estados, trans;
    if (dificuldadeN1 === "dificil") {
        estados = ["S0", "S1", "S1.5", "S2", "S3", "S4", "S5", "S6"];
        trans = {
            "S0": {"massa": "S1"},
            "S1": {"molho": "S1.5"},
            "S1.5": {"queijo": "S2"},
            "S2": {"pimentos": "S3", "fiambre": "S4"},
            "S3": {"bacon": "S5"},
            "S4": {"azeitonas": "S5"},
            "S5": {"pagar": "S6"},
            "S6": {}
    ...
    const fsmN1 = {
        estados: estados,
        inicial: "S0",
        final: dificuldadeN1 === "dificil" ? "S6" : "S3",
        trans: trans
    };

    let str = "";

    str += "RESUMO DA ATIVIDADE:\n";
    str += "Completas-te com sucesso a pizza seguindo as regras do autómato finito determinístico.\n";
    str += "A sequência de ingredientes seguiu a expressão regular especificada, resultando numa pizza válida.\n\n";

    str += "DETALHES DO AUTÓMATO:\n";
    str += "Q = { " + fsmN1.estados.join(", ") + " }\n";
    str += "q0 = " + fsmN1.inicial + "\n";
    str += "F = { " + fsmN1.final + " }\n";
    str += "\nExpressão Regular:\n";
    str += getRegexForDifficulty(dificuldadeN1) + "\n";
    str += "\nδ:\n";
    ...
    str += "\nPROCEDIMENTO DA TABELA δ:\n";
    str += "A tabela δ representa a função de transição do autómato finito determinístico.\n";
    str += "Cada linha da tabela mostra uma transição possível: (estado atual, símbolo de entrada) → próximo estado.\n";
    str += "Para processar uma sequência de símbolos, começa-se no estado inicial q0 e aplica-se as transições sucessivamente até ao final da sequência.\n";
    str += "Se o estado final for alcançado, a sequência é aceite; caso contrário, é rejeitada.\n";
    ...
    header.innerHTML = "<th>Estado</th><th>Token</th><th>Destino</th><th>Descrição</th>";
    table.appendChild(header);

    const descriptions = {
        "massa": "Adicionar massa à pizza",
        "molho": "Adicionar molho à pizza",
        "queijo": "Adicionar queijo à pizza",
        "pimentos": "Adicionar pimentos como topping",
        "fiambre": "Adicionar fiambre como topping",
        "azeitonas": "Adicionar azeitonas como topping",
        "bacon": "Adicionar bacon como topping",
        "cogumelos": "Adicionar cogumelos como topping",
        "milho": "Adicionar milho como topping",
        "cebola": "Adicionar cebola como topping",
        "oregão": "Adicionar orégão como topping",
        "ananas": "Adicionar ananás como topping",
        "pagar": "Efetuar pagamento e finalizar a pizza"
    };

    for (const e in fsmN1.trans) {
        for (const tok in fsmN1.trans[e]) {
            const row = document.createElement("tr");
            const desc = descriptions[tok] || `Transição com ${tok}`;
            row.innerHTML = `
                <td>${e}</td>
                <td>${tok}</td>
...
function desenharToppingN1(tok, pos) {
    ...
    if (icon) {
        ...
        img.setAttribute("href", "data:image/svg+xml;base64," + btoa(icon));
        img.setAttributeNS("http://www.w3.org/1999/xlink", "href", "data:image/svg+xml;base64," + btoa(icon));
...
const ingredientIcons = {
    "massa": '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g><circle cx="32" cy="32" r="24" fill="#f3d4a3" stroke="#c99f68" stroke-width="3"/><circle cx="32" cy="32" r="18" fill="#f8e2bd" stroke="#e2c48f" stroke-width="2"/></g></svg>',
    "molho": '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g><circle cx="32" cy="32" r="22" fill="#d94c3a" stroke="#a83228" stroke-width="3"/><circle cx="32" cy="32" r="16" fill="#e8634d" stroke="#c4473a" stroke-width="2"/></g></svg>',
    "queijo": '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g fill="#ffd966" stroke="#d4a000" stroke-width="2"><path d="M10 46 L32 10 L54 46 Z"/><circle cx="26" cy="34" r="3" fill="#e6bf3b"/><circle cx="38" cy="28" r="2.5" fill="#e6bf3b"/><circle cx="34" cy="40" r="2" fill="#e6bf3b"/></g></svg>',
    "pimentos": '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g fill="#2ecc71" stroke="#1f8f54" stroke-width="2"><path d="M20 24 C28 16, 44 16, 48 28 C52 40, 40 48, 30 44 C22 40, 12 34, 20 24 Z"/><rect x="18" y="18" width="6" height="8" fill="#1f8f54"/></g></svg>',
    "fiambre": '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g><circle cx="32" cy="32" r="20" fill="#ff9db5" stroke="#cc6e84" stroke-width="2"/><circle cx="24" cy="30" r="3" fill="#cc6e84"/><circle cx="40" cy="36" r="2.5" fill="#cc6e84"/></g></svg>',
    "azeitonas": '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g><circle cx="32" cy="32" r="20" fill="#2c3e50" stroke="#1a252f" stroke-width="2"/><circle cx="28" cy="28" r="3" fill="#34495e"/><circle cx="36" cy="36" r="2.5" fill="#34495e"/></g></svg>',
    "cogumelos": '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g><circle cx="32" cy="24" r="12" fill="#e74c3c" stroke="#c0392b" stroke-width="2"/><rect x="28" y="30" width="8" height="12" fill="#f39c12" stroke="#e67e22" stroke-width="1"/></g></svg>',
    "bacon": '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g><rect x="12" y="20" width="40" height="8" fill="#e74c3c" stroke="#c0392b" stroke-width="2"/><rect x="12" y="32" width="40" height="6" fill="#f39c12" stroke="#e67e22" stroke-width="1"/><rect x="12" y="38" width="40" height="6" fill="#e74c3c" stroke="#c0392b" stroke-width="1"/></g></svg>',
    "milho": '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g><rect x="28" y="12" width="8" height="40" fill="#f1c40f" stroke="#f39c12" stroke-width="2"/><circle cx="26" cy="20" r="2" fill="#e67e22"/><circle cx="38" cy="24" r="1.5" fill="#e67e22"/><circle cx="30" cy="32" r="2" fill="#e67e22"/><circle cx="34" cy="40" r="1.5" fill="#e67e22"/></g></svg>',
    "cebola": '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g><circle cx="32" cy="32" r="18" fill="#f39c12" stroke="#e67e22" stroke-width="2"/><circle cx="32" cy="32" r="12" fill="#f1c40f" stroke="#d4ac0d" stroke-width="1"/><circle cx="28" cy="28" r="2" fill="#e67e22"/><circle cx="36" cy="36" r="1.5" fill="#e67e22"/></g></svg>',
    "oregao": '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g fill="#27ae60" stroke="#1e8449" stroke-width="2"><circle cx="20" cy="20" r="4"/><circle cx="44" cy="16" r="3"/><circle cx="28" cy="36" r="3.5"/><circle cx="40" cy="44" r="2.5"/><circle cx="16" cy="40" r="3"/></g></svg>',
    "ananas": '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g><rect x="20" y="20" width="24" height="32" fill="#f1c40f" stroke="#f39c12" stroke-width="2"/><rect x="18" y="16" width="4" height="8" fill="#27ae60"/><rect x="26" y="16" width="4" height="6" fill="#27ae60"/><rect x="34" y="16" width="4" height="10" fill="#27ae60"/><rect x="42" y="16" width="4" height="8" fill="#27ae60"/><circle cx="24" cy="28" r="1.5" fill="#e67e22"/><circle cx="32" cy="32" r="1.5" fill="#e67e22"/><circle cx="40" cy="36" r="1.5" fill="#e67e22"/></g></svg>',
    "chocolate": '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g><rect x="16" y="16" width="32" height="32" fill="#8b4513" stroke="#654321" stroke-width="2"/><rect x="20" y="20" width="8" height="8" fill="#654321"/><rect x="32" y="24" width="6" height="6" fill="#654321"/><rect x="24" y="32" width="8" height="6" fill="#654321"/></g></svg>',
    "gelado": '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g><rect x="24" y="24" width="16" height="24" fill="#3498db" stroke="#2980b9" stroke-width="2"/><circle cx="32" cy="20" r="8" fill="#e74c3c" stroke="#c0392b" stroke-width="1"/><circle cx="32" cy="18" r="6" fill="#f39c12" stroke="#e67e22" stroke-width="1"/></g></svg>',
    "gomas": '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g><circle cx="20" cy="20" r="6" fill="#e74c3c"/><circle cx="44" cy="16" r="5" fill="#f39c12"/><circle cx="28" cy="36" r="7" fill="#9b59b6"/><circle cx="40" cy="44" r="4" fill="#1abc9c"/><circle cx="16" cy="40" r="5" fill="#e67e22"/></g></svg>',
    "rebucados": '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g><circle cx="20" cy="20" r="6" fill="#ff69b4"/><circle cx="44" cy="16" r="5" fill="#ff1493"/><circle cx="28" cy="36" r="7" fill="#dc143c"/><circle cx="40" cy="44" r="4" fill="#c71585"/><circle cx="16" cy="40" r="5" fill="#ffb6c1"/></g></svg>',
    "alface": '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g fill="#27ae60" stroke="#1e8449" stroke-width="2"><path d="M32 12 C40 8, 52 16, 48 28 C44 40, 32 36, 24 32 C16 28, 12 20, 20 16 C24 12, 28 10, 32 12 Z"/><circle cx="30" cy="24" r="2" fill="#1e8449"/><circle cx="38" cy="30" r="1.5" fill="#1e8449"/></g></svg>',
    "sushi": '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g><rect x="16" y="24" width="32" height="16" fill="#f8e2bd" stroke="#c99f68" stroke-width="2"/><rect x="20" y="28" width="8" height="8" fill="#e74c3c"/><rect x="32" y="28" width="6" height="8" fill="#27ae60"/><rect x="42" y="28" width="4" height="8" fill="#f39c12"/></g></svg>',
    "esparguete": '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g><path d="M12 20 Q32 12 52 20 Q48 28 32 24 Q16 28 12 20 Z" fill="#f39c12" stroke="#e67e22" stroke-width="2"/><circle cx="20" cy="18" r="1.5" fill="#e67e22"/><circle cx="32" cy="16" r="1.5" fill="#e67e22"/><circle cx="44" cy="18" r="1.5" fill="#e67e22"/><circle cx="24" cy="22" r="1" fill="#e67e22"/><circle cx="40" cy="22" r="1" fill="#e67e22"/></g></svg>',
    "melancia": '<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 64 64"><g><circle cx="32" cy="32" r="22" fill="#e74c3c" stroke="#c0392b" stroke-width="2"/><path d="M10 32 Q32 10 54 32 Q32 54 10 32 Z" fill="#27ae60" stroke="#1e8449" stroke-width="1"/><circle cx="24" cy="24" r="2" fill="#2ecc71"/><circle cx="40" cy="28" r="1.5" fill="#2ecc71"/><circle cx="32" cy="40" r="2" fill="#2ecc71"/></g></svg>'
};

function obterIconeIngrediente(nome) {
    return ingredientIcons[nome] || '<svg width="64" height="64"><circle cx="32" cy="32" r="16" fill="#ffcc55"></circle></svg>';
}
...
function getRegexForDifficulty(diff) {
    if (diff === "facil") {
        return "Massa (molho queijo | queijo) (pimentos|fiambre|azeitonas|cogumelos|bacon|milho|cebola|oregão|ananas|alface|sushi|esparguete|melancia)* pagar";
    }
    if (diff === "medio") {
        return "Massa (molho queijo | queijo) (pimentos|fiambre|azeitonas|cogumelos|bacon|milho|cebola|oregão|ananas|alface|sushi|esparguete|melancia)+ pagar";
    }
    if (diff === "dificil") {
        return "Massa molho queijo (pimentos bacon | fiambre azeitonas) (cogumelos|milho|cebola|oregão|ananas|alface|sushi|esparguete|melancia)* pagar";
    }
    return "";
}
...
function validateFinalPizzaByRegex(diff, seqArray) {
    ...
    if (!seq) return false;
    const valid_toppings = ['pimentos', 'fiambre', 'azeitonas', 'cogumelos', 'bacon', 'milho', 'cebola', 'oregao', 'ananas'];
    const toppings = valid_toppings.join('|');
    let pattern;
    if (diff === 'facil') {
        pattern = new RegExp('^massa(molhoqueijo|queijo)(' + toppings + ')*pagar$');
    } else if (diff === 'medio') {
        pattern = new RegExp('^massa(molhoqueijo|queijo)(' + toppings + ')+pagar$');
    } else if (diff === 'dificil') {
        pattern = new RegExp('^massamolhoqueijo(pimentosbacon|fiambreazeitonas)(' + toppings + ')*pagar$');
    } else {
        return false;
    }
...
function desenharFSM_N2() {
    ...
    fsm.estados.forEach((e, i) => {
        const angle = (i / numEstados) * 2 * Math.PI - Math.PI / 2; // Start from top
...
function renderIngredientesN2(base, distratores) {
    ...
    todos.forEach(tok => {
        ...
        iconDiv.style.display = "block";
        iconDiv.style.margin = "0 auto 6px auto";
        const canonical = normalizeIngredient(tok);
        const icon = obterIconeIngrediente(canonical);
        iconDiv.innerHTML = icon;
...
function mostrarFormalizacao_N2() {
    ...
    str += "RESUMO DA ATIVIDADE:\n";
    str += "Completas-te com sucesso o desafio interagindo com um autómato finito gerado aleatoriamente.\n";
    str += "A sequência de ingredientes seguiu as transições do autómato, alcançando o estado final.\n\n";
...
function gerarSequenciaN3() {
    const base = ["massa", "molho", "queijo", "pimentos", "fiambre", "azeitonas", "bacon", "cogumelos", "milho", "cebola", "oregão", "ananas", "pagar"];
...
function desenharAutomatoN3() {
    ...
    const numEstados = estadosN3.length;
    const centerX = 700;
    const centerY = 200;
    // Ajustar raio automaticamente baseado no número de estados
    const radius = Math.max(120, 250 - estadosN3.length * 15);

    // Posicionar estados em círculo para melhor visualização
    estadosN3.forEach((e, i) => {
        const angle = (i / estadosN3.length) * 2 * Math.PI + Math.PI / 2; // Começar de baixo
        const x = centerX + radius * Math.cos(angle);
        const y = centerY + radius * Math.sin(angle);
...
function mostrarFormalizacao_N3() {

    // Since level 3 doesn't have a formal text area, we can add the summary to a log message or create a new element
    // For now, let's add it to the log
    logN3("RESUMO DA ATIVIDADE:");
    logN3("Crias-te com sucesso um autómato finito determinístico que aceita a sequência de ingredientes fornecida.");
    logN3("O autómato processa a sequência completa, começando no estado inicial e terminando num estado final.");
    logN3("DETALHES DO AUTÓMATO CRIADO:");
    logN3("Estados: " + estadosN3.join(", "));
    logN3("Estado inicial: " + inicialN3);
    logN3("Estados finais: " + finaisN3.join(", "));
    logN3("Sequência aceita: " + sequenciaN3.join(" → "));
}
